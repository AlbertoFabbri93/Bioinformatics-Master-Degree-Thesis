...
for (res in names(macrophage_cells_markers_list)) {
  ...
  markers_df <- macrophage_cells_markers_list[[res]]
  
  # Filter markers to get the most significant up-regulated ones per cluster
  most_significant_markers <- markers_df %>%
    group_by(cluster) %>%
  ...
  macrophage_cells_markers_to_print[[key_markers]] <- most_significant_markers
  ...
  heatmap_plot <- DoHeatmap(
    ...
    title = paste("Macrophage cells clusters - Resolution:", res),
    subtitle = "Top 10 Differentially Expressed Genes per Cluster"
  )
  
  # Save the heatmap for this resolution in the output list
  key_heatmap <- paste0(res, "_heatmap")
  macrophage_cells_markers_to_print[[key_heatmap]] <- heatmap_plot
}
