...
This study aims to investigate the immune microenvironment of brain metastases from primary breast cancer. The analysis is performed on NanoString CosMx data from 7 patients.

## Set up the environment

### Packages

This analysis uses `renv` to manage the dependencies. `renv` is an R package that can be installed with:
...
If you want to check for package updates without installing them use `renv::update(check=TRUE)`.
If you want to install the updates use `renv::update()`.
If you want to only update a specific package use `renv::update("package_name")`.
...
library("CellChat")
# Conversion between Seurat and CellChat objects
...
tidy_seurat_object_assay <- function(seurat_object_mixed_features, assay_name) {
  ...
  filtered_metadata$Patient.ID <- as.factor(filtered_metadata$Patient.ID)
...
Here, cells with low quality data are removed. First, cells that have been categorized as red blood cells according to a previous study are removed. Then, cells with a very low number of expressed genes and cells with an extremely high expression of any gene are removed.
...
This code adds the ER (estrogen receptor) status of the patients to the metadata.
...
The cell sizes are calculated from the metadata. The width is calculated from the height and the aspect ratio. The width and height are then converted from pixels to micrometers using the conversion factor provided by NanoString.
...
Check for mitochondrial genes, there should not be any.
...
Insitutype  uses the raw counts, therefore it is useless to use the integrated data because Seurat does not provide an adjusted count matrix. The cells are annotated with the cell types from the reference profiles. Finally, some cell types are manually merged together.
...
The cell types are grouped into larger groups (e.g. "Myeloids", "Stromal cells", etc.)
...
Print some interactive tables with information about the cell typing done by Insitutyoe.
...
ist_summary_cell_types_count <- breast_cancer_patients_analyze@meta.data %>%
  dplyr::count(.data[[col_ist_ann]], name = "cell_count") %>%
...
ist_summary_cell_types_count_per_patient <- breast_cancer_patients_analyze@meta.data %>%
  dplyr::count(!!sym(col_ist_ann), Patient.ID) %>%
  group_by(Patient.ID) %>%
...
ist_summary_cell_groups_count <- breast_cancer_patients_analyze@meta.data %>%
  dplyr::count(.data[[col_cell_group]], name = "cell_count") %>%
  mutate(percentage = round((cell_count / sum(cell_count)) * 100, 3)) %>%
...
ist_summary_cell_groups_count_per_patient <- breast_cancer_patients_analyze@meta.data %>%
  dplyr::count(!!sym(col_cell_group), Patient.ID) %>%
  group_by(Patient.ID) %>%
  mutate(percentage = round(n / sum(n) * 100, 4)) %>%
...
Normalize the data using the SCTransform method provided by Seurat. SCTransform is a method for normalizing single-cell data that uses a regularized negative binomial regression model to account for technical noise and biological variability.
...
# Plot UMAPs with proteins expression
proteins_feature_plots_sct <- FeaturePlot(
  object = breast_cancer_patients_analyze,
  reduction = "sct_umap_patients",
  features = c("Mean.PanCK", "Mean.CD45", "Mean.CD68"),
  max.cutoff = "q95",
  min.cutoff = 0,
  combine = FALSE)
proteins_feature_plots_sct <- lapply(
  proteins_feature_plots_sct,
  function(x) x &
  NoLegend() &
  NoAxes() & 
  labs(
...
names(proteins_feature_plots_sct) <- glue('proteins_feature_plots_sct_{c("PanCK", "CD45", "CD68")}')
patients_ist_sct <- c(patients_ist_sct, proteins_feature_plots_sct)
...
Try also the old Seurat normalization workflow. The data is normalized, scaled, and PCA is performed.
...
Visualize protein expression data of the Insitutype clusters.
...
Plot the spatial plots with the cell types.
...
save_data(ist_cell_types_stamps_plots, file.path(dir_ist_plots, "Stamps"))
```
...
Add the Insitutype metadata to the original Seurat object. The original Seurat object will be used to create the various Seurat subsets.
...
The normal cells are the non-cancerous cells from the breast cancer patients.

```{r}
#| label: normal-cells-load-metadata

normal_cells_dir <- file.path(gl_img_dir, "Normal cells")
```

### Compute

Normalize the data and calculate the UMAP embeddings.
...
```{r}
#| label: normal-cells-create-object
#| eval: false
...
Plot UMAPs and other plots showing the distribution of the non-malignant cells across patients.
...
# Calculate the percentage of cell type per patient.
# The different cell types of every patient add up to 100%
normal_cells_percentages_per_patient <- breast_cancer_patients_normal_cells@meta.data %>%
  # Group by patient and cluster, then calculate percentage
  # Cells are also grouped by cell group in order to retain that column
  group_by(Patient.ID, `Cell type` = !!sym(col_ist_ann), `Cell group` = !!sym(col_cell_group)) %>%
  summarise(`Cell count` = n(), .groups = "drop") %>%
  group_by(Patient.ID) %>%
  mutate(`Within-patient %` = `Cell count` / sum(`Cell count`) * 100) %>%
  ungroup()
normal_cells_images[["normal_cells_percentages_per_patient"]] <- normal_cells_percentages_per_patient

# Calculate the mean and standard deviation from the percentages of each cell type
normal_cells_percentages_per_patient_statistics <- normal_cells_percentages_per_patient %>%
  # Calculate total number of cells, mean and sd for each cell type
  group_by(`Cell type`, `Cell group`) %>%
  summarize(
    `Tot cell count` = sum(`Cell count`),
    `Mean within-patient %` = mean(`Within-patient %`),
    `SD within-patient %` = sd(`Within-patient %`)
  )
normal_cells_images[["normal_cells_percentages_per_patient_statistics"]] <- normal_cells_percentages_per_patient_statistics

# Plot the percentages of the various types of cells (except cancer cells)
normal_cells_cluster_distribution_barplot <- normal_cells_percentages_per_patient %>%
  ...
  facet_grid(
    cols = ggplot2::vars(`Cell group`),
    scales = "free_x",
    space = "free",
  ) +
  labs(
    x = NULL, 
    y = "Within-patient % of non-malignant cells", 
...
normal_cells_cluster_distribution_dotplot <- normal_cells_percentages_per_patient %>%
  # Plot the data
  ggplot(
    aes(
      x = `Within-patient %`,
  ...
  ggforce::facet_col(
    facets = ggplot2::vars(`Cell group`),
    scales = "free_y",
    space = "free",
    drop = TRUE,
    strip.position = "top",
  ...
  geom_point(
    aes(
      x = `Mean within-patient %`,
      shape = "Mean",
    ),
    data = normal_cells_percentages_per_patient_statistics,
    size = 3,
    color = "black",
    alpha = 0.8,
  ) +
  ...
  geom_errorbarh(
    aes(
      xmin = `Mean within-patient %` - `SD within-patient %`, 
      xmax = `Mean within-patient %` + `SD within-patient %`, 
      y = `Cell type`,
  ...
  labs(
    x = "Within-patient % of non-malignant cells",
    y = NULL,
    title = "Non-malignant cells type distribution by patient",
    color = "Patient",
  ) +
  scale_color_manual(
    values = palette_patients,
  ) +
  scale_shape_manual(
    values = c("Mean" = 18),
    labels = c("Mean"),
  ...
  scale_linetype_manual(
    values = c("Standard Deviation" = "solid"),
  ...
  guides(
    ...
    shape = guide_legend(
      order = 2,
      title = "Statistics",
      title.theme = element_text(margin = margin(t = 10, b = 6)),
    ),
  ...
  theme(
    ...
    # Title plot is centered over the entire plot
    plot.title.position = "plot",
    # Change style of facets
...
normal_cells_type_umap_plot <- DimPlot(
  ...
  labs(
    title = "Non-malignant cell types"
  ) +
  guides(
    color = guide_legend(
      override.aes = list(size=2),
...
normal_cells_group_umap_plot <- DimPlot(
  ...
  labs(
    title = "Non-malignant cell groups"
  ) +
  theme(
    plot.title = element_text(size = 20),
    legend.text = element_text(size = 12),
...
normal_cells_clusters_custom_markers <- c(
  #Astrocytes = "GFAP",
  `B cell`= "IGKC",
  `Dendritic cell` = "CD74",
  `Endothelial cell` = "COL4A1",
  `Fibroblast` = "FN1",
  `Macrophage` = "CD68",
  # `Microglia` = "ERBB2",
  # `Monocyte` = "LTF",
...
normal_cells_custom_markers_feature_plot <- FeaturePlot(
  object = breast_cancer_patients_normal_cells,
  features = normal_cells_clusters_custom_markers,
  reduction = "sct_umap_normal_cells",
  ncol = 3
  ) & 
  NoLegend() &
...
for (i in seq_along(normal_cells_clusters_custom_markers)) {
  norm_mark <- names(normal_cells_clusters_custom_markers)[i]
  normal_cells_custom_markers_feature_plot[[i]] <-
    normal_cells_custom_markers_feature_plot[[i]] +
    labs(subtitle = norm_mark) +
    theme(
      plot.subtitle = element_text(size = 14, hjust = 0.5)
    )
}
normal_cells_images[["normal_cells_custom_markers_feature_plot"]] <- normal_cells_custom_markers_feature_plot
...
normal_cells_distribution_dotplot_attr <- list(
  width = 25,
  height = 20,
  units = "cm",
  dpi = 600
...
normal_cells_images <- set_plot_attributes(
  normal_cells_images,
  normal_cells_distribution_dotplot_attr,
  c("normal_cells_cluster_distribution_dotplot")
)
# Save the normal cells plots
save_data(normal_cells_images, normal_cells_dir)
```

### Info

Create interactive tables with the number of cells and the percentage of each cell type and group, globally and per patient.

```{r}
#| label: normal-cells-info

normal_cells_info <- list()

# CELL TYPES
# Create a tibble with cell count and percentage of every cluster
summary_normal_cells_types_count <- breast_cancer_patients_normal_cells@meta.data %>%
  dplyr::count(.data[[col_ist_ann]], name = "cell_count") %>%
  mutate(percentage = round((cell_count / sum(cell_count)) * 100, 3)) %>%
  dplyr::rename(cell_type = .data[[col_ist_ann]]) %>%
  as_tibble()
normal_cells_info[["summary_normal_cells_types_count"]] <- summary_normal_cells_types_count

# Create a table with the cell count and percentage divided by patient
summary_normal_cells_types_count_per_patient <- breast_cancer_patients_normal_cells@meta.data %>%
  dplyr::count(!!sym(col_ist_ann), Patient.ID) %>%
  group_by(Patient.ID) %>%
  mutate(percentage = round(n / sum(n) * 100, 4)) %>%
  arrange(Patient.ID, !!sym(col_ist_ann)) %>%
  dplyr::rename(cell_type = !!sym(col_ist_ann)) %>%
  as_tibble()
normal_cells_info[["summary_normal_cells_types_count_per_patient"]] <- summary_normal_cells_types_count_per_patient

# CELL GROUPS
# Create a tibble with cell count and percentage of every group
summary_normal_cells_groups_count <- breast_cancer_patients_normal_cells@meta.data %>%
  dplyr::count(.data[[col_cell_group]], name = "cell_count") %>%
  mutate(percentage = round((cell_count / sum(cell_count)) * 100, 3)) %>%
  dplyr::rename(cell_group = .data[[col_cell_group]]) %>%
  as_tibble()
normal_cells_info[["summary_normal_cells_groups_count"]] <- summary_normal_cells_groups_count

# Create a table with the cell count and percentage divided by patient
summary_normal_cells_groups_count_per_patient <- breast_cancer_patients_normal_cells@meta.data %>%
  dplyr::count(!!sym(col_cell_group), Patient.ID) %>%
  group_by(Patient.ID) %>%
  mutate(percentage = round(n / sum(n) * 100, 4)) %>%
  arrange(Patient.ID, !!sym(col_cell_group)) %>%
  dplyr::rename(cell_group = !!sym(col_cell_group)) %>%
  as_tibble()
normal_cells_info[["summary_normal_cells_groups_count_per_patient"]] <- summary_normal_cells_groups_count_per_patient

for (table in normal_cells_info) {
  print(datatable(table))
}
```
...
Find the markers of the non-malignant cells clusters. The markers are calculated using the Wilcoxon rank-sum test. Plot the most significant markers on a heatmap.

```{r}
#| label: normal-cells-markers-load

# Load normal cells object from memory
normal_cells_markers_data <- readRDS(file.path(gl_obj_dir, "normal_cells_markers_data.rds"))
```

```{r}
#| label: normal-cells-markers-computation
#| eval: false

# Set identity to the clusters whose markers we want to find
Idents(breast_cancer_patients_normal_cells) <- col_ist_ann

# Find all markers of the normal clusters
normal_cells_markers_data <- FindAllMarkers(
  breast_cancer_patients_normal_cells,
  test.use = "wilcox",
  assay = "SCT",
  only.pos = FALSE,
...
```{r}
#| label: normal-cells-markers-plot

# List to save and print
normal_cells_markers_to_print <- list()

# Filter markers to get the most significant up-regulated ones per cluster
most_significant_normal_cells_markers <- normal_cells_markers_data %>%
  group_by(cluster) %>%
  dplyr::filter(avg_log2FC > 1 & p_val_adj < 0.05) %>%
  slice_head(n = 10) %>%
  ungroup()
normal_cells_markers_to_print[["most_significant_normal_cells_markers"]] <- most_significant_normal_cells_markers

# Print
print(normal_cells_markers_to_print)
```

```{r}
#| label: normal-cells-markers-save
#| eval: false

# Save the data
saveRDS(normal_cells_markers_data, file.path(gl_obj_dir, "normal_cells_markers_data.rds"))
save_data(normal_cells_markers_to_print, normal_cells_dir)
```

```{r}
#| label: normal-cells-markers-complex-heatmap

# Info to save
normal_cells_complex_heatmap_list <- list()

# Unique is used to eliminate duplicated genes, this is done automatically by Seurat DoHeatmap
features <- unique(most_significant_normal_cells_markers$gene)

# Extract the expression data for the selected genes from the scaled layer
normal_cells_expr_matrix <- as.matrix(
  GetAssayData(
    breast_cancer_patients_normal_cells, 
    layer = "scale.data", 
    assay = "SCT"
...
normal_cells_clusters <- Idents(breast_cancer_patients_normal_cells)
# Create vector with unique and sorted cell types
unique_cell_types <- sort(unique(normal_cells_clusters))
...
palette_expression_level = circlize::colorRamp2(
  c(min(normal_cells_expr_matrix), median(normal_cells_expr_matrix), min(2.5, max(normal_cells_expr_matrix))),
  c("#FF00FF", "#000000", "#FFFF00"))

# Create the heatmap with ComplexHeatmap
normal_cells_clust_diff_expr_genes_complex_heatmap <- Heatmap(
  matrix = normal_cells_expr_matrix,
  row_order = features,
  name = "expression",
  column_split = factor(normal_cells_clusters, levels = unique_cell_types),
  ...
  top_annotation = HeatmapAnnotation(
    ...
    cluster = normal_cells_clusters,
    # Second annotation - patients
    patient = breast_cancer_patients_normal_cells@meta.data$Patient.ID,
    # Color palettes
...
draw(normal_cells_clust_diff_expr_genes_complex_heatmap, merge_legend = TRUE)

normal_cells_complex_heatmap_list[["normal_cells_clust_diff_expr_genes_complex_heatmap"]] <- recordPlot()
```

```{r}
#| label: normal-cells-markers-heatmap-save
#| eval: false

# Define and set complex heatmap attributes
complex_heatmap_attrs <- list(width = 15, height = 20, units = "cm", dpi = 1000)
normal_cells_complex_heatmap_list <- set_plot_attributes(normal_cells_complex_heatmap_list, complex_heatmap_attrs)
# Save ComplexHeatmap
save_data(normal_cells_complex_heatmap_list, normal_cells_dir)
```

## Macrophages

The macrophages are a type of immune cell that play a key role in the immune response. Here I subset the main Seurat object to create a new one with only the macrophages.
...
# Macrophages main folder
macrophage_dir <- file.path(gl_img_dir, "Macrophages")
```

```{r}
#| label: macrophage-cells-subset
#| eval: false
...
Do a subclustering of the macrophages to find different subtypes. Try different resolutions to find the best one.

```{r}
#| label: macrophage-cells-metadata-columns

# Name of the metadata column with the macrophage clusters
...
Plot the UMAP of the macrophages with the different clustering resolutions.
...
macrophage_cells_patient_id_plot <- DimPlot(
  ...
  cols = palette_patients) +
  labs(
    title = "Patient ID clusters",
    subtitle = "Macrophages") +
  theme(
    plot.title = element_text(size = 20),
    plot.subtitle = element_text(size = 16, hjust = 0.5)
  ) &
...
Show tables with statistics about the macrophage clusters.

```{r}
#| label: macrophage-cells-info

# Create a list to store the information
macrophages_info <- list()

# Create a table with the cell count and percentage divided by patient
macrophage_cells_count_per_patient <- breast_cancer_patients_macrophage_cells@meta.data %>%
  dplyr::count(!!sym(col_ist_ann), Patient.ID) %>%
  mutate(total_cells = sum(n)) %>%
  group_by(Patient.ID) %>%
  mutate(percentage = round(n / total_cells * 100, 2)) %>%
  arrange(Patient.ID, !!sym(col_ist_ann)) %>%
  dplyr::rename(cell_type = !!sym(col_ist_ann)) %>%
  dplyr::select(-total_cells) %>%
  as_tibble()
macrophages_info[["macrophage_cells_count_per_patient"]] <- macrophage_cells_count_per_patient

macrophage_cells_count_per_cluster <- breast_cancer_patients_macrophage_cells@meta.data %>%
  dplyr::count(!!sym(col_ist_ann), !!sym(col_macrophage_cells_chosen_cluster)) %>%
  mutate(total_cells = sum(n)) %>%
  group_by(!!sym(col_macrophage_cells_chosen_cluster)) %>%
  mutate(percentage = round(n / total_cells * 100, 2)) %>%
  arrange(!!sym(col_macrophage_cells_chosen_cluster), !!sym(col_ist_ann)) %>%
  dplyr::rename(cell_type = !!sym(col_ist_ann)) %>%
  dplyr::select(-total_cells) %>%
  as_tibble()
macrophages_info[["macrophage_cells_count_per_cluster"]] <- macrophage_cells_count_per_cluster

print(macrophages_info)
```

```{r}
#| label: macrophage-cells-save-clusters-data
#| eval: false

# Save the macrophage cells info
save_data(c(macrophage_cells_plots_to_save, macrophages_info), file.path(macrophage_dir, "Clusters"))
```

### Markers

Find the markers of the macrophage clusters. The markers are calculated using the Wilcoxon rank-sum test. Plot the most significant markers on a heatmap.
...
for (i in seq_along(macrophage_cells_markers_list)) {
  
  # Get the resolution string
  res_str <- names(macrophage_cells_markers_list)[i]
  # Get only the resolution number
  res_val <- macrophage_cells_seq_res[i]
  
  # Print the current resolution being analyzed
  print(paste("Processing resolution:", res_val))
  
  # Heatmaps title
  macrophage_cells_heatmap_title = paste("Macrophage cells clusters - Resolution:", res_val)
  
  # Get marker data for the current resolution
  macrophage_cells_markers_df <- macrophage_cells_markers_list[[res_str]]
  ...
  key_markers <- paste0(res_str, "_most_significant_markers")
  ...
  macrophage_key_heatmap <- paste0(res_str, "_heatmap")
  ...
  macrophage_cells_clusters <-
    setNames(
      factor(
        as.vector(
          breast_cancer_patients_macrophage_cells@meta.data[[res_str]]
        )
      ),
  ...
  macrophage_cells_clust_diff_expr_genes_complex_heatmap <- Heatmap(
    ...
    top_annotation = HeatmapAnnotation(
      ...
      # Color palettes
      col = list(cluster = macrophage_cell_type_colors, patient = palette_patients),
      show_legend = c(FALSE, TRUE),
      gap = unit(1, "mm"),
      simple_anno_size = unit(2, "mm"),
      annotation_name_gp = gpar(fontsize = 6)
  ...
  # Save the heatmap for this resolution in the output list
  macrophage_key_complex_heatmap <- paste0(res_str, "_complex_heatmap")
...
```{r}
#| label: macrophage-cells-markers-heatmap-save
#| eval: false

# Other cells markers folder
macrophage_cells_markers_dir <- file.path(macrophage_dir, "Markers")
# Define and set complex heatmap attributes
...
Plot the cells on the UMAP according to their pseudotime, which is a measure of the progression of the cells through a biological process.

```{r}
#| label: macrophage-cells-cds-load
...
save_data(macrophage_cells_pseudotime_images, file.path(macrophage_dir, "Pseudotime"))
```

## Fibroblast

The fibroblasts are a type of connective tissue cell that play a key role in the structure and function of tissues.

```{r}
#| label: fibroblast-cells-metadata

# Fibroblast main folder
fibroblast_dir <- file.path(gl_img_dir, "Fibroblasts")
```

### Compute

Here I subset the main Seurat object to create a new one with only the fibroblasts.

```{r}
#| label: fibroblast-cells-load

# Load fibroblast cells object from memory
breast_cancer_patients_fibroblast_cells <- readRDS(file.path(gl_obj_dir, "breast_cancer_patients_fibroblast_cells.rds"))
```

```{r}
#| label: fibroblast-cells-subset
#| eval: false

breast_cancer_patients_fibroblast_cells <- 
  breast_cancer_patients_filt_cells[, which(x = ist_ann_column == "Fibroblast")]
```

### Cluster

Do a subclustering of the fibroblasts to find different subtypes. Try different resolutions to find the best one.

```{r}
#| label: fibroblast-cells-metadata-columns

# Name of the metadata column with the other clusters
col_fibroblast_cells_clust_res_prefix <- "fibroblast_cells_clusters_res_"
# Find clusters of fibroblast cells
# The default resolution is 1, increase it to get more clusters or decrease it to get fewer clusters
# Try a range of resolutions
fibroblast_cells_seq_res <- seq(0.4, 0.7, by = 0.1)
# Create custom cluster names based on the actual resolution values
fibroblast_cells_cluster_seq_res <-
  glue::glue("{col_fibroblast_cells_clust_res_prefix}{fibroblast_cells_seq_res}")
```

```{r}
#| label: fibroblast-cells-compute
#| eval: false

# After subsetting a Seurat object it is good to normalize it again
breast_cancer_patients_fibroblast_cells <- SCTransform(
  breast_cancer_patients_fibroblast_cells,
  return.only.var.genes = FALSE,
  assay = "RNA",
  new.assay.name = "SCT",
...
# Run PCA
breast_cancer_patients_fibroblast_cells <- RunPCA(
  breast_cancer_patients_fibroblast_cells,
  reduction.key = "PC_",
  reduction.name = "pca",
  seed.use = 33,
  verbose = TRUE)
# Generate variance elbow plots
generate_variance_elbow_plots(breast_cancer_patients_fibroblast_cells, "pca", 50)
# Dimension to use for subsequent analysis, decided lokking at the elbow plot
fibroblast_cells_dims <- 1:29
# Find neighbors of other cells
# Number of PCA dimensions used in Seurat's FindNeighbors function
#
# Using MORE dimensions:
...
# May miss subtle biological signals
# Computationally faster
breast_cancer_patients_fibroblast_cells <- FindNeighbors(
  breast_cancer_patients_fibroblast_cells,
  reduction = "pca",
  dims = fibroblast_cells_dims,
  verbose = TRUE)

# Find clusters at different resolutions
breast_cancer_patients_fibroblast_cells <- FindClusters(
  breast_cancer_patients_fibroblast_cells,
  resolution = fibroblast_cells_seq_res,
  cluster.name = fibroblast_cells_cluster_seq_res,
  random.seed = 5,
  verbose = TRUE)

# Run UMAP
breast_cancer_patients_fibroblast_cells <- RunUMAP(
  breast_cancer_patients_fibroblast_cells,
  reduction = "pca",
  reduction.key = "UMAP_",
  reduction.name = "umap",
...
Plot the UMAP of the fibroblasts with the different clustering resolutions. Also, plot the protein data and the distribution of clusters across patients.

```{r}
#| label: fibroblast-cells-plot

fibroblast_cells_plots_to_save <- list()

# View UMAP with other cells at different clustering resolutions
fibroblast_cells_different_resolutions <- DimPlot(
  breast_cancer_patients_fibroblast_cells,
  reduction = "umap",
  ncol = 2,
  group.by = fibroblast_cells_cluster_seq_res,
  label = TRUE,
  ) & 
  NoLegend() & 
  NoAxes() & 
...
fibroblast_cells_plots_to_save[["fibroblast_cells_different_resolutions"]] <- fibroblast_cells_different_resolutions

# View how resolution affects the number of clusters
fibroblast_cells_clustree <- clustree(
  breast_cancer_patients_fibroblast_cells@meta.data,
  prefix = col_fibroblast_cells_clust_res_prefix)
fibroblast_cells_plots_to_save[["fibroblast_cells_clustree"]] <- fibroblast_cells_clustree

# View UMAP with fibroblasts and patient clusters
fibroblast_cells_patient_id_plot <- DimPlot(
  breast_cancer_patients_fibroblast_cells,
  reduction = "umap",
  label = TRUE,
  label.box = TRUE,
  ...
  NoLegend() &
  NoAxes()
fibroblast_cells_plots_to_save[["fibroblast_cells_patient_id_plot"]] <- fibroblast_cells_patient_id_plot

# Chosen resolution
chosen_fibroblast_cells_cluster_resolution <- 0.4
# Chosen cluster
col_fibroblast_cells_chosen_cluster <- glue::glue("{col_fibroblast_cells_clust_res_prefix}{chosen_fibroblast_cells_cluster_resolution}")
# Set identity to other clusters
Idents(breast_cancer_patients_fibroblast_cells) <- col_fibroblast_cells_chosen_cluster

# Generate DimPlot of chosen resolution
fibroblast_cells_chosen_resolution_plot <- DimPlot(
  breast_cancer_patients_fibroblast_cells,
  reduction = "umap",
  group.by = col_fibroblast_cells_chosen_cluster,
  label = TRUE,
  label.box = TRUE,
  label.size = 5) +
  labs(
    title = "Fibroblast Clusters"
  ) +
  theme(
    plot.title = element_text(size = 20),
  ) &
  ...
  NoAxes()
# Print DimPlot chosen resolution
fibroblast_cells_plots_to_save[["fibroblast_cells_chosen_resolution_plot"]] <- fibroblast_cells_chosen_resolution_plot

# Other cells cluster tibble column name
stat_col_fibroblast_cells_clusters <- "Fibroblast clusters"
# Calculate statistics for the other cells clusters
fibroblast_cells_cluster_statistics <- breast_cancer_patients_fibroblast_cells@meta.data %>%
  dplyr::rename(!!sym(stat_col_fibroblast_cells_clusters) := !!sym(col_fibroblast_cells_chosen_cluster)) %>%
  # Group by patient and cluster, then calculate percentage
  group_by(Patient.ID, !!sym(stat_col_fibroblast_cells_clusters)) %>%
  summarise(Count = n(), .groups = "drop") %>%
  group_by(Patient.ID) %>%
  mutate(Percentage = Count / sum(Count) * 100) %>%
  ungroup() %>%
...
# Print tibble with the number of cells in each cluster per patient
fibroblast_cells_count_clust_patient <- 
  fibroblast_cells_cluster_statistics %>%
    # Specify id_cols so the Count column is dropped
    pivot_wider(
      names_from = Patient.ID,
      values_from = Count,
      id_cols = !!sym(stat_col_fibroblast_cells_clusters)
  ) %>%
  # Replace NA values with 0 so that the total cells per cluster can be calculated
  mutate(across(2:8, ~replace_na(.x, 0))) %>%
  group_by(!!sym(stat_col_fibroblast_cells_clusters)) %>%
  mutate(`Tot. clust. cells` = sum(across(where(is.numeric)))) %>%
  ungroup() %>%
  # Sort according to the Tumor cluster column
  arrange(!!sym(stat_col_fibroblast_cells_clusters))
fibroblast_cells_plots_to_save[["fibroblast_cells_count_clust_patient"]] <-
  fibroblast_cells_count_clust_patient

# Plot the distribution of other cell clusters by patient
fibroblast_cells_cluster_distribution_plot <- fibroblast_cells_cluster_statistics %>%
  # Plot the data
  ggplot(aes(x = Count, y = !!sym(stat_col_fibroblast_cells_clusters))) +
  geom_rect(data = unique(fibroblast_cells_cluster_statistics["Fibroblast clusters"]),
            aes(ymin = as.numeric(`Fibroblast clusters`) - 0.4,
                ymax = as.numeric(`Fibroblast clusters`) + 0.4,
                xmin = -Inf,
                xmax = Inf),
            fill = "grey90", alpha = 0.3, inherit.aes = FALSE) +
  # Add dots for each patient
  ...
  theme_minimal() +
  labs(x = "Number of Cells", y = "Cluster", 
       title = "Fibroblast Clusters Distribution by Patient",
       color = "Patient") +
  theme(legend.position = "right") +
  scale_color_brewer(palette = "Set1") +
  guides(
...
fibroblast_cells_plots_to_save[["fibroblast_cells_cluster_distribution_plot"]] <-
  fibroblast_cells_cluster_distribution_plot

# Plot UMAPs with proteins expression
fibroblast_cells_feature_plots <- FeaturePlot(
  object = breast_cancer_patients_fibroblast_cells,
  reduction = "umap",
  features = c("Mean.PanCK", "Mean.CD45", "Mean.CD68"),
  max.cutoff = "q95",
  min.cutoff = 0,
  label = TRUE,
  combine = FALSE)
fibroblast_cells_feature_plots <- lapply(
  fibroblast_cells_feature_plots,
  function(x) x &
  NoLegend() &
  NoAxes() &
  labs(
...
names(fibroblast_cells_feature_plots) <- glue('fibroblast_cells_feature_plot_{c("PanCK", "CD45", "CD68")}')
fibroblast_cells_plots_to_save <- c(fibroblast_cells_plots_to_save, fibroblast_cells_feature_plots)

# Violin plot with the expression of panCK, CD45 and CD68 for each Insitutype clusters
fibroblast_cells_clust_protein_expr <- VlnPlot(
  breast_cancer_patients_fibroblast_cells,
  assay = "SCT",
  layer = "data",
  features = c("Mean.PanCK", "Mean.CD45", "Mean.CD68"),
...
fibroblast_cells_clust_protein_expr <- lapply(
  fibroblast_cells_clust_protein_expr,
  function(x) x & NoLegend() & labs(subtitle = "Fibroblast Clusters"))
names(fibroblast_cells_clust_protein_expr) <- glue('Fibroblast_cells_{c("PanCK", "CD45", "CD68")}')
fibroblast_cells_plots_to_save <- c(fibroblast_cells_plots_to_save, fibroblast_cells_clust_protein_expr)

print(fibroblast_cells_plots_to_save)
```

### Info

Show tables with statistics about the fibroblast clusters.

```{r}
#| label: fibroblast-cells-info

# Create a list to store the information
fibroblasts_info <- list()

# Total number of fibroblast cells
count_total_fibroblast = nrow(breast_cancer_patients_fibroblast_cells@meta.data)

# Create a table with the cell count and percentage divided by patient
fibroblast_cells_count_per_patient <- breast_cancer_patients_fibroblast_cells@meta.data %>%
  dplyr::count(!!sym(col_ist_ann), Patient.ID) %>%
  group_by(Patient.ID) %>%
  mutate(percentage = round(n / count_total_fibroblast * 100, 2)) %>%
  arrange(Patient.ID, !!sym(col_ist_ann)) %>%
  dplyr::rename(cell_type = !!sym(col_ist_ann)) %>%
  as_tibble()
fibroblasts_info[["fibroblast_cells_count_per_patient"]] <- fibroblast_cells_count_per_patient

# Initialize a list to store marker results for each resolution
fibroblast_cells_count_per_cluster_list <- list()
# Loop over each resolution's cluster metadata column
for (res in fibroblast_cells_cluster_seq_res) {
  fibroblast_cells_count_per_cluster <- breast_cancer_patients_fibroblast_cells@meta.data %>%
    dplyr::count(!!sym(col_ist_ann), !!sym(res)) %>%
    group_by(!!sym(res)) %>%
    mutate(percentage = round(n / count_total_fibroblast * 100, 2)) %>%
    arrange(!!sym(res), !!sym(col_ist_ann)) %>%
    dplyr::rename(cell_type = !!sym(col_ist_ann)) %>%
    as_tibble()
  
  # Save the heatmap for this resolution in the output list
  fibroblast_key_count_per_cluster <- paste0(res, "_count_per_cluster")
  fibroblast_cells_count_per_cluster_list[[fibroblast_key_count_per_cluster]] <- fibroblast_cells_count_per_cluster
}
# Merge the two dataframe lists
fibroblasts_info <- c(fibroblasts_info, fibroblast_cells_count_per_cluster_list)

# Print total cells to console
print(glue("Total fibroblast cells: {count_total_fibroblast}"))
# Print dataframes
print(fibroblasts_info)
```

```{r}
#| label: fibroblast-cells-save-rds
#| eval: false

# Save the Seurat object
saveRDS(breast_cancer_patients_fibroblast_cells, file.path(gl_obj_dir, "breast_cancer_patients_fibroblast_cells.rds"))
```

```{r}
#| label: fibroblast-cells-save-clusters-data
#| eval: false

# Save the macrophage cells info
save_data(c(fibroblast_cells_plots_to_save, fibroblasts_info), file.path(fibroblast_dir, "Clusters"))
```

### Markers

Find the markers of the fibroblast clusters.Plot the most significant markers on a heatmap.

```{r}
#| label: fibroblast-cells-markers-load

# Load other cells object from memory
fibroblast_cells_markers_list <- readRDS(file.path(gl_obj_dir, "fibroblast_cells_markers_list.rds"))
```

```{r}
#| label: fibroblast-cells-markers-computation
#| eval: false

# Initialize a list to store marker results for each resolution
fibroblast_cells_markers_list <- list()

# Loop over each resolution's cluster metadata column
for (res in fibroblast_cells_cluster_seq_res) {
  # Use the group.by parameter to specify the clustering metadata column
  fibroblast_clust_res_markers <- FindAllMarkers(
    breast_cancer_patients_fibroblast_cells,
    group.by = res,
    test.use = "wilcox",
    assay = "SCT",
  ...
  # Save the markers for the current resolution in the list
  fibroblast_cells_markers_list[[res]] <- fibroblast_clust_res_markers
}
```

```{r}
#| label: fibroblast-cells-markers-save
#| eval: false

# Save the data
saveRDS(fibroblast_cells_markers_list, file.path(gl_obj_dir, "fibroblast_cells_markers_list.rds"))
```

```{r}
#| label: fibroblast-cells-markers-plots

# List with data to save
fibroblast_cells_markers_to_save <- list()
# List with complex heatmaps
fibroblast_cells_complex_heatmaps <- list()

# Loop over each resolution in the list of marker results
for (i in seq_along(fibroblast_cells_markers_list)) {
  
  # Get the resolution string
  res_str <- names(fibroblast_cells_markers_list)[i]
  # Get only the resolution number
  res_val <- fibroblast_cells_seq_res[i]
  
  # Print the current resolution being analyzed
  print(paste("Processing resolution:", res_val))
  
  # Heatmaps title
  fibroblast_cells_heatmap_title = paste("Fibroblast cells clusters - Resolution:", res_val)
  
  # Get marker data for the current resolution
  fibroblast_cells_markers_df <- fibroblast_cells_markers_list[[res_str]]
  
  # Filter markers to get the most significant up-regulated ones per cluster
  most_significant_fibroblast_cells_markers <- fibroblast_cells_markers_df %>%
    group_by(cluster) %>%
    dplyr::filter(avg_log2FC > 1 & p_val_adj < 0.05) %>%
    slice_head(n = 10) %>%
    ungroup()
  print(most_significant_fibroblast_cells_markers)
  
  # Save the filtered markers for this resolution in the output list
  key_markers <- paste0(res_str, "_most_significant_markers")
  fibroblast_cells_markers_to_save[[key_markers]] <- most_significant_fibroblast_cells_markers
  
  ### SEURAT HEATMAP
  # Create a heatmap using the genes from the filtered markers
  fibroblast_heatmap_plot <- DoHeatmap(
    breast_cancer_patients_fibroblast_cells,
    features = most_significant_fibroblast_cells_markers$gene,
    group.by = res_str,
    assay = "SCT",
    label = TRUE,
    size = 2,
    ...
    title = fibroblast_cells_heatmap_title,
    subtitle = "Top 10 Differentially Expressed Genes per Cluster"
  )
  print(fibroblast_heatmap_plot)
  
  # Save the heatmap for this resolution in the output list
  fibroblast_key_heatmap <- paste0(res_str, "_heatmap")
  fibroblast_cells_markers_to_save[[fibroblast_key_heatmap]] <- fibroblast_heatmap_plot
  
  ### COMPLEX HEATMAP
  # Unique is used to eliminate duplicated genes, this is done automatically by Seurat DoHeatmap
  unique_fibroblast_cells_markers <- unique(most_significant_fibroblast_cells_markers$gene)

  # Extract the expression data for the selected genes from the scaled layer
  fibroblast_cells_expr_matrix <- as.matrix(
    GetAssayData(
      breast_cancer_patients_fibroblast_cells, 
      layer = "scale.data", 
      assay = "SCT"
    )[
      unique_fibroblast_cells_markers,
    ]
  )

  # Prepare cell type annotations
  fibroblast_cells_clusters <-
    setNames(
      factor(
        as.vector(
          breast_cancer_patients_fibroblast_cells@meta.data[[res_str]]
        )
      ),
      rownames(breast_cancer_patients_fibroblast_cells@meta.data))

  # Create vector with unique and sorted cell types
  unique_fibroblast_cell_types <- sort(unique(fibroblast_cells_clusters))
  
  # Create color palette for cell types
  fibroblast_cell_type_colors <- setNames(
    gg_color_hue(length(unique_fibroblast_cell_types)),
    unique_fibroblast_cell_types
  )

  # Recreate the Seurat expression scale
  palette_fibroblast_expression_level = circlize::colorRamp2(c(
    min(fibroblast_cells_expr_matrix),
    median(fibroblast_cells_expr_matrix),
    min(2.5, max(fibroblast_cells_expr_matrix))),
    c("#FF00FF", "#000000", "#FFFF00"))

  # Create the heatmap with ComplexHeatmap
  fibroblast_cells_clust_diff_expr_genes_complex_heatmap <- Heatmap(
    matrix = fibroblast_cells_expr_matrix,
    row_order = unique_fibroblast_cells_markers,
    name = "expression",
    column_split = factor(fibroblast_cells_clusters, levels = unique_fibroblast_cell_types),
    # Do not cluster rows or columns otherwise row_order/column_order are ignored
    cluster_rows = FALSE,
    cluster_columns = FALSE,
    column_title_rot = 0,
    ...
    row_names_gp = gpar(fontsize = 5),
    # The column title can be either a title for all the columns or a vector with a name for every column
    column_title = unique_fibroblast_cell_types,
    column_title_gp = gpar(fontsize = 7),
    # Add the annotation bars on top
    top_annotation = HeatmapAnnotation(
      # Give a name to the annotation, not used on the actual plot
      ...
      which = "column",
      # First annotation - clusters
      cluster = fibroblast_cells_clusters,
      # Second annotation - patients
      patient = breast_cancer_patients_fibroblast_cells@meta.data$Patient.ID,
      # Color palettes
      col = list(cluster = fibroblast_cell_type_colors, patient = palette_patients),
      show_legend = c(FALSE, TRUE),
      gap = unit(1, "mm"),
      simple_anno_size = unit(2, "mm"),
      annotation_name_gp = gpar(fontsize = 6)
    ...
    # Color scale for expression
    col = palette_fibroblast_expression_level,
    # Otherwise you get random vertical white lines where there should not be any
    use_raster = FALSE
  )
  # Put the legends on the right in a single column
  draw(
    object = fibroblast_cells_clust_diff_expr_genes_complex_heatmap,
    merge_legend = TRUE,
    column_title = fibroblast_cells_heatmap_title)
  
  # Save the heatmap for this resolution in the output list
  fibroblast_key_complex_heatmap <- paste0(res_str, "_complex_heatmap")
  fibroblast_cells_complex_heatmaps[[fibroblast_key_complex_heatmap]] <- recordPlot()
  fibroblast_cells_markers_to_save <- c(fibroblast_cells_markers_to_save, fibroblast_cells_complex_heatmaps)
}
```

```{r}
#| label: fibroblast-cells-markers-heatmap-save
#| eval: false

# Other cells markers folder
fibroblast_cells_markers_dir <- file.path(fibroblast_dir, "Markers")
# Define and set complex heatmap attributes
fibroblast_cells_complex_heatmap_attrs <- list(width = 18, height = 13, units = "cm", dpi = 600)
fibroblast_cells_markers_to_save <- set_plot_attributes(
  list_name = fibroblast_cells_markers_to_save,
  attr_list = fibroblast_cells_complex_heatmap_attrs,
  plot_names = names(fibroblast_cells_complex_heatmaps))
# Save the data
save_data(fibroblast_cells_markers_to_save, fibroblast_cells_markers_dir)
```

## Other cells

The other cells are the cells that have not been identified by Seurat using the reference profiles. Most of these cells are expected to be malignant.

```{r}
#| label: other-cells-load-metadata

other_cells_dir <- file.path(gl_img_dir, "Other cells")
```

### Cluster

Here I subset the main Seurat object to create a new one with only the other cells. Then I run the clustering algorithm on the other cells.
...
```{r}
#| label: other-cells-create-object
#| eval: false
...
Generate plots to investigate the clusters.
...
other_cells_features_umap <- c("Mean.PanCK", "Mean.CD45", "Mean.CD68", "CD63")
...
other_cells_feature_plots <- FeaturePlot(
  object = breast_cancer_patients_other_cells,
  reduction = "umap",
  features = other_cells_features_umap,
  max.cutoff = "q95",
  min.cutoff = 0,
  label = TRUE,
  combine = FALSE)
other_cells_feature_plots <- lapply(
  other_cells_feature_plots,
  function(x) x &
  NoLegend() &
  NoAxes() &
  labs(
    subtitle = "Malignant cells"
  ) &
  theme(
    plot.title = element_text(size = 20),
    plot.subtitle = element_text(size = 16, hjust = 0.5)
...
names(other_cells_feature_plots) <- glue('other_cells_feature_plot_{other_cells_features_umap}')
other_cells_plots_to_save <- c(other_cells_plots_to_save, other_cells_feature_plots)
...
save_data(other_cells_plots_to_save, file.path(other_cells_dir, "Clusters"))
```
...
Find the markers of the other cells clusters. Plot the most significant markers on a heatmap.
...
```{r}
#| label: other-cells-markers-heatmap-save
#| eval: false

# Other cells markers folder
other_cells_markers_dir <- file.path(other_cells_dir, "Markers")
# Save the markers
...
# Percentage of cells in each group per patient
all_cells_percent_cell_groups_patients <- 
  breast_cancer_patients_analyze@meta.data %>%
  # Calculate the percentage of cells in each group
  group_by(Patient.ID, !!sym(col_ist_merge_group)) %>%
  summarise(Count = n(), .groups = "drop") %>%
  group_by(Patient.ID) %>%
  mutate(Percentage = Count / sum(Count) * 100) %>%
  ungroup()
all_cells_images[["all_cells_percent_cell_groups_patients"]] <- all_cells_percent_cell_groups_patients

# Plot the percentage of cells in each group per patient
all_cells_percent_cell_groups_patients_plot <-
all_cells_percent_cell_groups_patients %>%
  ggplot(
    aes(
      x = Patient.ID,
      y = Percentage,
      fill = !!sym(col_ist_merge_group)
    )
  ) +
  geom_bar(stat = "identity", width = 0.7, color = NA) +
  scale_fill_manual(values = palette_cell_types) +
  labs(
    title = "Percentage of cells in each group per patient",
    y = "Percentage (%)", 
    x = "Patient", 
    fill = "Cell group") +
  theme_minimal() +
...
all_cells_images[["all_cells_percent_cell_groups_patients_plot"]] <- all_cells_percent_cell_groups_patients_plot
...
Gene Set Enrichment Analysis (GSEA) is a method to determine whether a set of genes shows statistically significant differences between two biological states. In this case, we will use GSEA to analyze the other cells clusters.

### GSEA objects

Calculate the GSEA results for each tumor cluster. The results are saved in a list.
...
Generate GSEA plot and dataframe for each tumor cluster. The plots and dataframes are saved in different lists.
...
Merge normal and tumor subclusters annotations.
...
cell_types_count <- breast_cancer_patients_analyze@meta.data %>%
  dplyr::count(.data[[col_cell_tumor_cluster_subtype]], name = "cell_count") %>%
  mutate(percentage = round((cell_count / sum(cell_count)) * 100, 3)) %>%
...
The spatial niches are areas in the cores that have a similar cell composition.

### Compute

The niches are calculated using the spatial images and the cell phenotypes.
...
# Number of niche to calculate, if wanting more than 12 niches change the color palette below
niches_nums <- c(6, 9, 12)
```
...
Set the metadata used for plotting.
...
# Create the color palette for the niches, maximum 12 colors are available in the brewer palette
palette_niches <- generate_colors_lookup_table(
  breast_cancer_patients_analyze,
...
Plot the cores with the cells highlighted according to the niches they belong to.
...
niches_stamps_plots <- generate_spatial_plots(
  ...
  legend_fill = "Niches",
  color_lookup_table = palette_niches)
# Display the plots
...
Calculate some statistics about the niches like the cell composition.
...
for (niche in niches) {
  ...
  niche_stat <- cell_type_counts %>%
    ...
    # Combine the cells whose cluster name starts with "Tumor"
    dplyr::mutate(
      !!sym(col_cell_tumor_cluster_subtype) := ifelse(
        grepl("^Tumor", !!sym(col_cell_tumor_cluster_subtype)),
        "Malignant cell",
    ...
    # Filter out cells whose percentage is below the cutoff
    dplyr::filter(percentage >= niche_cell_type_cutoff) %>%
    # Filter out cells in the unknown cluster
    dplyr::filter(!!sym(col_cell_tumor_cluster_subtype) != "Unknown")
...
for (niche in niches) {
  ...
  niche_barplot <- niche_data %>%
    ...
    labs(
      ...
      # caption = glue(
      #   "Cutoff value: {niche_cell_type_cutoff}%, the cells types with a percentage below the cutoff are removed.
    ...
    # Hard-code the y axis limits
    scale_y_continuous(limits = c(0, 100)) +
    # Make space for the count number above the bars, not necessary if hard-coding the y axis limits
    # expand_limits(y = max(niche_data$percentage) + 5) +
    theme_minimal() +
    theme(
...
# Create the bar plot
niches_patient_dotplot <- niches_patient_counts %>%
  ggplot(
    aes(
      x = Patient.ID,
      y = percentage,
      color = !!sym(col_niches_plot)
    )
  ) +
  geom_rect(
    data = unique(niches_patient_counts["Patient.ID"]),
    aes(
      xmin = as.numeric(Patient.ID) - 0.4,
  ...
  labs(
    title = paste("Niches distribution across patients"),
    x = "Patient",
    y = "Percentage (%)"
  ) +
  ...
  scale_color_manual(values = palette_niches)
# Add to list
niches_patient_data[["niches_patient_dotplot"]] <- niches_patient_dotplot

niches_filtered <- niches_patient_counts %>%
  dplyr::filter(!!sym(col_niches_plot) %in% c("1", "8"))

# Create bar chart
niches_filtered_bar_plot <- niches_filtered %>%
  ggplot(
    aes(
      x = Patient.ID,
      y = percentage,
      fill = !!sym(col_niches_plot)
    )
  ) +
  # Add bars with position_dodge to group them by patient
  geom_bar(
    stat = "identity", 
  ...
  labs(
    title = "Distribution of niches 1 and 8 across patients",
    x = "Patient",
    y = "Percentage (%)"
  ) +
  ...
  # Keep only the colors for niches 1 and 8 from your palette
  scale_fill_manual(values = palette_niches[c("1", "8")])
niches_patient_data[["niches_filtered_bar_plot"]] <- niches_filtered_bar_plot
...
CellChat is a tool for inferring and analyzing cell-cell communication networks from single-cell transcriptomic data. In this case, we will use CellChat to analyze the cell-cell communication between the different cell types.

### Distances

Calculate the distances between the various non-malignant cell types and the closest tumor cells.
...
Infer the three types of cell-cell communication: secreted signaling, ECM-receptor signaling and cell-cell contact. Use both the RNA and the spatial data to infer the cell-cell communication.
...
cellchat_datatype_subtitle <- c("RNA", "RNA + Spatial")
# Set the datatype to be used
cellchat_datatype_index <- 2
cellchat_datatype = cellchat_datatypes[cellchat_datatype_index]
...
cellchat_DB_subtitle <- c("All signaling", "Cell-cell contact", "Secreted signaling", "ECM receptors")
# Set the database to be used
cellchat_DB_index <- 1
CellChatDB_suffix <- cellchat_databases[cellchat_DB_index]
...
Plot the cell-cell communication network and the signaling pathways showing significant communications. Also, plot the interaction strength and the number of interactions between cell groups.
...
# Interactions used
cellchat_interactions_name <- cellchat_DB_subtitle[cellchat_DB_index]
# Datatype used
cellchat_datatype_name <- cellchat_datatype_subtitle[cellchat_datatype_index]
```
...
netVisual_heatmap(
  ...
  font.size = 9,
  color.use = palette_cellchat,
  # Make the colors different between the two types of heatmap, darker color for interactions close to 0
...
netVisual_bubble(
  object = cellchat,
  title.name = glue("Significant interactions\n{cellchat_interactions_name}\n{cellchat_datatype_name}"),
  sources.use = NULL,
  targets.use = NULL,
  remove.isolate = TRUE,
  signaling = NULL,
