...
expr_matrix <- as.matrix(
  GetAssayData(
    ...
    layer = "data", 
    assay = "SCT")[
    most_significant_other_cells_markers$gene, 
    ]
  )
# Scale the rows
zero_var_rows <- apply(expr_matrix, 1, var) == 0
expr_matrix_scaled <- expr_matrix
expr_matrix_scaled[!zero_var_rows, ] <- t(scale(t(expr_matrix[!zero_var_rows, ])))
expr_matrix_scaled[zero_var_rows, ] <- 0
...
# Define palette for expression level
valid_values <- expr_matrix_scaled[!is.na(expr_matrix_scaled) & !is.infinite(expr_matrix_scaled)]
palette_expression_level <- colorRamp2(
  c(min(valid_values), median(valid_values), max(valid_values)),
  c("#FF00FF", "#000000", "#FFFF00")
)

# Save png
png(here("test.png"),width=15,height=20,units="cm",res=1000)

# Create the heatmap with ComplexHeatmap
Heatmap(
  matrix = expr_matrix_scaled,
  name = "Expression",
  column_split = factor(cell_types),
  cluster_rows = TRUE,
  ...
  column_title = "Top 10 Differentially Expressed Genes per Cluster",
...
# Save the data
save_data(other_cells_markers_to_print, here(images_dir, "Other cells markers"))
```
...
num_tumor_clusters <- length(grep("^Tumor", unique(breast_cancer_patients_filt_cells@meta.data[[col_cell_tumor_cluster_subtype]])))
