...
col_other_cells_clust_res_prefix <- "other_cells_clusters_res"
...
tumor_cluster_seq_res <- glue::glue("{col_other_cells_clust_res_prefix}{seq_res}")
```
...
# Chosen cluster
chosen_tumor_cluster <- glue::glue("{col_other_cells_clust_res_prefix}{chosen_tumor_cluster_resolution}")
...
names(tumor_clust_protein_expr) <- glue('Other_cells_{c("PanCK", "CD45", "CD68")}')
...
breast_cancer_patients_tumor_cells@meta.data %<>%
  ...
  "Tumor" ~ glue("Tumor {get(chosen_tumor_cluster)}"),
...
```{r}
#| label: annotate-other-cells-save
#| eval: false
...
breast_cancer_patients_filt_cells <- AddMetaData(
  ...
  metadata = FetchData(
    object = breast_cancer_patients_analyze,
    vars = col_ist_merge_type),
  col.name = col_ist_merge_type
)
...
# Count the number of tumor clusters by looking at the column "col_ist_merge" and counting the number of unique cluster names that starts with tumor
num_tumor_clusters <- length(grep("^Tumor", unique(breast_cancer_patients_filt_cells@meta.data[[cell_subcluster_col]])))
