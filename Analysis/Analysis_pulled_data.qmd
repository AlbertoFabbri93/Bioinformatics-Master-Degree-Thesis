...
breast_cancer_patients_mixed_features <- readRDS(file.path(gl_obj_dir, "breast.cancer.patients.original.rds"))
...
ist_summary_cell_types_count <- breast_cancer_patients_analyze@meta.data %>%
  ...
  dplyr::rename(cluster = .data[[col_ist_ann]]) %>%
  as_tibble()
...
save_data(ist_info, file.path(gl_img_dir, "Insitutype"))
```
...
# Save the Insitutype plots
save_data(patients_ist_sct, file.path(gl_img_dir, "Insitutype"))
save_data(patients_ist_rna, file.path(gl_img_dir, "Insitutype"))
save_data(insitutype_data, file.path(gl_img_dir, "Insitutype"))
```
...
# Set the databased that should be used
CellChatDB_suffix <- "all_protein_signalling"
CellChatDB_used <- glue("CellChatDB_{CellChatDB_suffix}")

# Load cellchat object
cellchat <- readRDS(path.file(gl_obj_dir, glue("cellchat_{CellChatDB_suffix}.rds")))
```
...
saveRDS(cellchat, file.path(gl_obj_dir, glue("cellchat_{CellChatDB_suffix}.rds")))
```

### Plot

```{r}
#| label: cellchat-plots
#| eval: false

# CellChat images folder
cellchat_images <- file.path(images_dir, "CellChat", CellChatDB_suffix)
# Check if the directory exists; if not, create it (including parent directories if needed)
if (!dir.exists(here(cellchat_images))) {
  dir.create(here(cellchat_images), recursive = FALSE)
}
...
# Circle plot
png(filename = here(cellchat_images,"cellchat_circleplot_count.png"), width = 15, height = 18, units = "cm", res = 1000)
netVisual_circle(
...
dev.off()
# Heatmap
png(filename = here(cellchat_images,"cellchat_heatmap_count.png"), width = 15, height = 14, units = "cm", res = 1000)
netVisual_heatmap(
  object = cellchat,
  measure = "count",
...
# Circle plot
png(filename = here(cellchat_images,"cellchat_circleplot_weight.png"), width = 15, height = 18, units = "cm", res = 1000)
netVisual_circle(
...
dev.off()
# Heatmap
png(filename = here(cellchat_images,"cellchat_heatmap_weight.png"), width = 15, height = 14, units = "cm", res = 1000)
netVisual_heatmap(
  object = cellchat,
  measure = "weight",
  font.size = 7,
...
# Create a subfolder to save images
cellchat_cell_types_output_dir <- file.path(cellchat_images, "Cell_types")
# Create the directory if it does not exist
if (!dir.exists(here(cellchat_cell_types_output_dir))) {
  dir.create(here(cellchat_cell_types_output_dir), recursive = FALSE)
}
for (i in 1:nrow(cellchat_weight_mat)) {
  ...
  png(
    filename = here(
      cellchat_cell_types_output_dir,
      glue("Cellchat_circleplot_{gsub(' ', '_', rownames(cellchat_weight_mat)[i])}.png")),
    width = 15,
    height = 18,
    units = "cm",
    res = 1000)
...
for (pathway in pathways_with_significant_signaling) {
  png(filename=here(cellchat_images,glue("Signaling_pathway_network_{pathway}.png")), width = 15, height = 8, units = "cm", res = 1000)
...
setEPS()
postscript(file = here(cellchat_images,"cellchat_bubbleplot_interactions.eps"))
png(filename = here(cellchat_images,"cellchat_bubbleplot_interactions.png"), width = 28, height = 15, units = "cm", res = 1000)
...
save_data(cellchat_dfs, here(cellchat_images))
```