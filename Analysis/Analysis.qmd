...
breast_cancer_patients_no_rbc_no_spatial <- breast_cancer_patients_no_rbc
...
mt_gene_count <- sum(grepl("^MT-", rownames(breast_cancer_patients_no_rbc_no_spatial[["RNA"]])))
if (mt_gene_count > 0) {
...
vln_plot_raw <- VlnPlot(
  breast_cancer_patients_no_rbc_no_spatial,
  assay = "RNA",
  layer = "counts",
...
KRT_panCK_coexpression[[3]] <- KRT_panCK_coexpression[[3]] + labs(title = "PanCK, KRT genes co-expression")
...
cells_all_phenotypes_color_palette <- generate_colors_lookup_table(
  ...
  color_palette = unname(polychrome()))
