...
integration_plot_list <- c()

# Check if there are mitochondrial genes
mt_gene_count <- sum(grepl("^MT-", rownames(breast_cancer_patients[["RNA"]])))
if (mt_gene_count > 0) {
...
integration_plot_list <- c(integration_plot_list, list(VlnPlot(
  breast_cancer_patients,
  features = c("nFeature_RNA", "nCount_RNA"),
  ncol = 2,
  pt.size = 0) + labs(title = "Gene count and expression level")))

# Normalize all the data
breast_cancer_patients <- SCTransform(breast_cancer_patients, vars.to.regress = c("nFeature_RNA"), return.only.var.genes = FALSE, verbose = FALSE)
# Run PCA
breast_cancer_patients <- RunPCA(breast_cancer_patients, verbose = FALSE, assay = "SCT")
# Generate variance elbow plots
generate_variance_elbow_plots(breast_cancer_patients, "pca", 50)
# Run UMAP
breast_cancer_patients <- RunUMAP(breast_cancer_patients, reduction = "pca", dims = 1:18, verbose = FALSE)
# View UMAP with all patients together
integration_plot_list <- c(integration_plot_list, list(DimPlot(breast_cancer_patients, reduction = "umap", group.by = "Patient.ID")))

# Split the global Seurat object by patient
breast_cancer_patients_split <- SplitObject(breast_cancer_patients, split.by = "Patient.ID")
bcp_split_project_names <- names(breast_cancer_patients_split)
# Set the project name for each patient
breast_cancer_patients_split <- lapply(bcp_split_project_names, function(seurat_object_name) {
  breast_cancer_patients_split[[seurat_object_name]]@project.name <- seurat_object_name
  breast_cancer_patients_split[[seurat_object_name]]
})

# RUN SCTransform for every patient
breast_cancer_patients_split <- lapply(breast_cancer_patients_split, function(x) {
  x <- SCTransform(x, vars.to.regress = c("nFeature_RNA"), verbose = FALSE)
  x <- RunPCA(x, verbose = FALSE, assay = "SCT")
  return(x)
})
# Find integration features
features <- SelectIntegrationFeatures(object.list = breast_cancer_patients_split, nfeatures = 1000)
# Prepare for integration
breast_cancer_patients_split <- PrepSCTIntegration(object.list = breast_cancer_patients_split, anchor.features = features)
...
# Run pca and umap
breast_cancer_patients_integrated <- RunPCA(breast_cancer_patients_integrated, verbose = TRUE)
breast_cancer_patients_integrated <- RunUMAP(breast_cancer_patients_integrated, dims = 1:10, verbose = TRUE)
# View UMAP with all patients together after per patient normalization
integration_plot_list <- c(integration_plot_list, list(DimPlot(
...
integration_ist_color_table <- generate_colors_lookup_table(breast_cancer_patients_integrated, ist_cluster_var, known_clusters_colors)
# Generate the umap with the unannotated clusters
DimPlot(
  object = breast_cancer_patients_integrated,
  group.by = ist_cluster_var,
  reduction = "umap",
  # cluster_name = ist_cluster_name,
  cols = integration_ist_color_table) +
  labs(title = "UMAP with Insitutype clusters")
create_cluster_summary(breast_cancer_patients_integrated, ist_cluster_var)
get_cluster_info(breast_cancer_patients_integrated, ist_cluster_var)
# Prepare object to run differential expression on SCT assay
breast_cancer_patients_integrated <- PrepSCTFindMarkers(breast_cancer_patients_integrated, assay = "SCT", verbose = TRUE)
# Find the most significant markers using the integrated data normalized with SCT
marks <- find_most_significant_markers(breast_cancer_patients_integrated, ist_cluster_var, "SCT")
print(integration_plot_list)
breast_cancer_patients_tumor_cells <- extract_clusters(breast_cancer_patients_integrated, ist_cluster_var, c("a", "b", "c", "d", "e", "f"))
breast_cancer_patients_tumor_cells <- SCTransform(breast_cancer_patients_tumor_cells, vars.to.regress = c("nFeature_RNA", "nCount_RNA", "Patient.ID"), verbose = FALSE)
# Run UMAP and PCA
...
DimPlot(
  ...
  cols = integration_ist_color_table)
p1 = DimPlot(breast_cancer_patients_tumor_cells, reduction = "umap_tumor_cells", label = TRUE, group.by = "Patient.ID")
p2 = DimPlot(breast_cancer_patients_tumor_cells, reduction = "umap_tumor_cells", label = TRUE, group.by = ist_cluster_var,
  cols = integration_ist_color_table)
p1 + p2
```
