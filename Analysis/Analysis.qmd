...
breast_cancer_patients_tumor_cells <- FindClusters(
  breast_cancer_patients_tumor_cells,
  resolution = 1.0,
  cluster.name = tumor_cluster_name,
  random.seed = 5,
  verbose = TRUE
)
# Run UMAP
...
tumor_cells_clusters_plot <- DimPlot(
  breast_cancer_patients_tumor_cells,
  reduction = "umap_tumor_cells",
  label = TRUE,
  group.by = tumor_cluster_name) +
  labs(title = "Tumor cells clusters")
tumor_cells_patient_id_plot + tumor_cells_clusters_plot

# Prepare object to run differential expression on SCTtumor assay
# breast_cancer_patients_tumor_cells <- PrepSCTFindMarkers(
#   breast_cancer_patients_tumor_cells,
#   assay = "SCTtumors",
#   verbose = TRUE)

# Set identity to tumor clusters
Idents(breast_cancer_patients_tumor_cells) <- tumor_cluster_name
...
# Generate summy statistics for the Insitutype clusters
create_cluster_summary(breast_cancer_patients_tumor_cells, tumor_cluster_name)
get_cluster_info(breast_cancer_patients_tumor_cells, tumor_cluster_name)
```
