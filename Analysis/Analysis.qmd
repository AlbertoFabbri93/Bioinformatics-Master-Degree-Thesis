...
### Remove spatial informations

```{r}
#| label: remove-spatial-informations

# Create a new Seurat object by copying the original
breast_cancer_patients_filt_cells_no_spatial <- breast_cancer_patients_filt_cells

# Save the spatial information to an object
spatial_information <- breast_cancer_patients_filt_cells@images

# Remove the spatial information from the new Seurat object
breast_cancer_patients_filt_cells_no_spatial@images <- list()
```
...
mt_gene_count <- sum(grepl("^MT-", rownames(breast_cancer_patients_filt_cells_no_spatial[["RNA"]])))
if (mt_gene_count > 0) {
...
vln_plot_raw <- VlnPlot(
  breast_cancer_patients_filt_cells_no_spatial,
  assay = "RNA",
  layer = "counts",
...
breast_cancer_patients_split <- SplitObject(breast_cancer_patients_filt_cells_no_spatial, split.by = "Patient.ID")
...
# Integrate the data
# After integration using SCT-normalized counts, the data slot will contain non-centered Pearson residuals and the scale.data slot will contain the centered residuals.
# Transforming post-integration residuals to corrected counts is not supported/tested/encouraged
...
```{r}
#| label: qa-after-integration-reductions
...
```{r}
#| label: qa-after-integration-plots

after_integration_plots <- list()

# View UMAP with all patients together
# If the integration has been done properly, the patients' cells should be mixed together
UMAP_patients_integrated <- DimPlot(
  breast_cancer_patients_integrated,
...
labs(
  title = "UMAP after per-patient normalization",
) +
# Remove all the axes elements
theme(
...
# Customize the titles for each plot
vln_plot_feat_count[[1]] <- vln_plot_feat_count[[1]] + labs(subtitle = "Gene count")
vln_plot_feat_count[[2]] <- vln_plot_feat_count[[2]] + labs(subtitle = "Total expression level")
# Add global title
vln_plot_feat_count <- vln_plot_feat_count + plot_annotation(title = "After normalization")
# Save the plot
after_integration_plots[["Violin_plot_features_count_integrated"]] <- vln_plot_feat_count

# Check the quantity and the expression level of genes per patient after normalization
vln_plot_feat_count_patients <- VlnPlot(
  breast_cancer_patients_integrated,
  assay = "integrated",
  layer = "data",
  features = c("nFeature_SCT","nCount_SCT"),
...
# Customize the titles for each plot
vln_plot_feat_count_patients[[1]] <- vln_plot_feat_count_patients[[1]] + labs(subtitle = "Gene count")
vln_plot_feat_count_patients[[2]] <- vln_plot_feat_count_patients[[2]] + labs(subtitle = "Total expression level")
# Add global title
vln_plot_feat_count_patients <- vln_plot_feat_count_patients + plot_annotation(title = "After normalization")
# Save the plot
after_integration_plots[["Violin_plot_patients_features_count_integrated"]] <- vln_plot_feat_count_patients

# QC the normalization
feature_plot_feat_count <- FeaturePlot(
  breast_cancer_patients_integrated,
  features = c("nFeature_RNA", "nCount_RNA"),
  reduction = "integrated_umap",
  combine = TRUE)
# Save the plot
after_integration_plots[["Feature_plot_features_count_integrated"]] <- feature_plot_feat_count

# Display the plots
print(after_integration_plots)
```
...
breast_cancer_patients_integrated$ist_semisup_clusters_ann <- case_match(
  ...
  .default = base::unname(breast_cancer_patients_integrated$ist_semisup_clusters)
)
# Generate the umap with the annotated clusters
insitutype_clust_v4int <- DimPlot(
...
insitutype_data[["insitutype_clust_v4int"]] <- insitutype_clust_v4int

# Generate summary statistics for the Insitutype clusters
insitutype_data[["insitutype_clust_summary_patients"]] <- create_cluster_summary_per_patient(breast_cancer_patients_integrated, ist_cluster_var)
...
ist_ann_cluster_statistics <- breast_cancer_patients_integrated@meta.data %>%
  group_by(ist_semisup_clusters_ann) %>%
  summarise(
...
```{r}
#| label: insitutype-save-plots
#| eval: false

# Save the Insitutype plots
save_plots(insitutype_data, here(images_dir, "Insitutype"))
```
...
immune_cells_cluster_distribution_barplot <- immune_cells_cluster_statistics %>%
  ...
  ggplot(aes(x = !!sym(ist_cluster_var), y = Percentage, fill = Patient.ID)) +
...
save_plots(immune_cells_images, here(images_dir, "Immune cells"))
```
...
save_plots(tumor_plots_to_save, here(images_dir, "Tumor clusters"))
```

### Markers

```{r}
#| label: tumor-cells-markers
...
# Save the data
save_plots(tumor_markers_to_print, here(images_dir, "Tumor markers"))
```
...
for (cluster in names(GSEA_results)) {
  gsea_simplified <- GSEA_results[[cluster]]
  ...
  if (nrow(gsea_simplified@result) > 0) {
    ...
    print(paste("No significant pathways found for cluster", cluster))
  }
}
```
...
# Save the GSEA plots
save_plots(GSEA_plots, here(images_dir, "GSEA plots"))
# Save the GSEA tibbles
save_plots(GSEA_tibbles, here(images_dir, "GSEA tables"))
# Save the GSEA results
saveRDS(GSEA_results, here(objects_dir, "GSEA_results.rds"))
```
...
```{r}
#| label: merge-all-cells

# Merge the immune and tumor cells into a single Seurat object
breast_cancer_patients_all_cells <- merge(
  breast_cancer_patients_immune_cells,
  breast_cancer_patients_tumor_cells)

# Copy tumor clusters from the tumor cells seurat object
breast_cancer_patients_all_cells$tumor_cluster <- breast_cancer_patients_tumor_cells[[chosen_tumor_cluster]]

# Merge the insitutype phenotyping with the tumor clusters
breast_cancer_patients_all_cells@meta.data <- breast_cancer_patients_all_cells@meta.data %>%
  mutate(cell_cluster = if_else(
    ist_semisup_clusters_ann != "Tumor",
    ist_semisup_clusters_ann,
    paste("Tumor", breast_cancer_patients_all_cells$tumor_cluster, sep = " ")
))
print(glue("Number of cell phenotypes (Insitutype + Tumor clusters): {length(unique(breast_cancer_patients_all_cells@meta.data$cell_cluster))} cells"))
...
breast_cancer_patients_all_cells <- BuildNicheAssay(
  object = breast_cancer_patients_all_cells, 
  fov = "LU001FFP03",
...
cells_all_phenotypes_color_palette <- generate_colors_lookup_table(
  breast_cancer_patients_all_cells,
...
# Save the plots
save_plots(cell_types_stamps_plots, here(images_dir, "Integration", "Stamps_with_clusters"))
```
...
# Save the plots
save_plots(niches_stamps_plots, here(images_dir, "Integration", "Stamps_with_niches"))
```
...
cell_type_counts <- breast_cancer_patients_all_cells@meta.data %>%
...
niches <- sort(unique(breast_cancer_patients_all_cells$niches))
# Print the niches
print(sort(niches))
...
for (niche in niches) {
  ...
  # Create the bar plot
  niche_barplot <- ggplot(niche_data, aes(x = cell_cluster, y = percentage, fill = cell_cluster)) +
...
# Count and calculate percentage of cell types per niche
niches_patient_counts <- breast_cancer_patients_all_cells@meta.data %>%
...
# Save the plots
save_plots(niches_barplot_list, here(images_dir, "Niches_statistics"))
save_plots(niches_patient_data, here(images_dir, "Niches_statistics"))
```
...
save_plots(patient_1_plots, get_patient_dir_img("1"), images_ext)
```
...
save_plots(patient_2_plots, get_patient_dir_img("2"), images_ext)
```
...
save_plots(patient_3_plots, get_patient_dir_img("3"), images_ext)
```
...
save_plots(patient_4_plots, get_patient_dir_img("4"), images_ext)
```
...
save_plots(patient_5_plots, get_patient_dir_img("5"), images_ext)
```
...
save_plots(patient_6_plots, get_patient_dir_img("6"), images_ext)
```
...
save_plots(patient_7_plots, get_patient_dir_img("7"), images_ext)
```
...
save_plots(compare_patients_to_save, compare_patients_subfolder, images_ext)
```
