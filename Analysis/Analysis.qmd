...
```{r}
#| label: ins-io-metadata-columns

# Name of the column with the raw insitutype clusters
raw_insitutype_col_name <- "patients_ist_io_semisup_clusters"
# Name of the metadata column with the annotated insitutype clusters
ann_insitutype_io_col_name <- "patients_ist_io_semisup_clusters_ann"
```
...
reference_profile <- io_profiles_matrix
...
breast_cancer_patients_analyze@meta.data %<>%
  mutate(
    "{ann_insitutype_io_col_name}" := case_match(
...
patients_ist_sct_plot <- DimPlot(
  object = breast_cancer_patients_analyze,
  group.by = ann_insitutype_io_col_name,
...
insitutype_data[["insitutype_clust_summary_patients"]] <- create_cluster_summary_per_patient(breast_cancer_patients_analyze, ann_insitutype_io_col_name)
...
ist_ann_cluster_statistics <- breast_cancer_patients_analyze@meta.data %>%
  group_by(!!sym(ann_insitutype_io_col_name)) %>%
  summarise(
...
breast_cancer_patients_filt_cells <- AddMetaData(
  ...
  metadata = FetchData(
    object = breast_cancer_patients_analyze,
    vars = ann_insitutype_io_col_name),
  col.name = ann_insitutype_io_col_name
)
```
...
breast_cancer_patients_immune_cells@meta.data %<>%
   mutate("{cell_type_col}" := cell_groups[.data[[ann_insitutype_io_col_name]]])
```
...
immune_cells_cluster_statistics <- breast_cancer_patients_immune_cells@meta.data %>%
  ...
  group_by(Patient.ID, !!sym(ann_insitutype_io_col_name), !!sym(cell_type_col)) %>%
...
immune_cells_cluster_distribution_barplot <- immune_cells_cluster_statistics %>%
  ...
  ggplot(aes(x = !!sym(ann_insitutype_io_col_name), y = Percentage, fill = Patient.ID)) +
...
immune_cells_cluster_distribution_dotplot <- immune_cells_cluster_statistics %>%
  # Plot the data
  ggplot(aes(x = Percentage, y = !!sym(ann_insitutype_io_col_name))) +
  ...
  geom_errorbarh(aes(xmin = MeanPercentage - SDPercentage, 
                     ...
                     y = !!sym(ann_insitutype_io_col_name),
...
# Set identity
Idents(breast_cancer_patients_immune_cells) <- ann_insitutype_io_col_name
# Vector of immune cells clusters to highlight
immune_clusters_show <- c("Macrophage", "Monocyte")
# Create a list with a vector of the cells in each immune cluster
immune_clusters_show_cells <- lapply(immune_clusters_show, function(cluster) {
  breast_cancer_patients_immune_cells %>%
    WhichCells(., ident = cluster)
})

# For every heterogeneous cluster, create a DimPlot with the cells from that cluster and the cells in the immune clusters
heterogeneous_clusters_umaps <- lapply(seq_along(heterogeneous_clusters_cells), function(i) {
  ...
  # Merge cells from the tumor and immune cells into a vector
  all_cells <- c(heterogeneous_clusters_cells[[i]], unlist(immune_clusters_show_cells))
...
save_data(heterogeneous_clusters_plots, here(images_dir, "Tumor clusters"))
```
...
Merge immune and tumor subclusters
...
breast_cancer_patients_filt_cells@meta.data %<>%
  mutate("{cell_subcluster_col}" := if_else(
    get(ann_insitutype_io_col_name) != "Tumor",
    get(ann_insitutype_io_col_name),
...
cell_type_counts <- breast_cancer_patients_filt_cells@meta.data %>%
  group_by(niches, !!sym(cell_subcluster_col)) %>%
  summarise(count = n()) %>%
  mutate(percentage = (count / sum(count)) * 100) %>%
  arrange(.by_group = TRUE)
# Print the cell type counts and percentages
niches_statistics[["cell_type_counts"]] <- cell_type_counts
...
for (niche in niches) {
  ...
  niche_data <- cell_type_counts %>%
    dplyr::filter(niches == niche) %>%
    dplyr::mutate(
    ...
    dplyr::group_by(!!sym(cell_subcluster_col)) %>%
    dplyr::summarize(
...
# Save niches plots to list
niches_statistics <- c(niches_statistics, niches_barplot_list)

# Display all niches bar plots
print(niches_statistics)
```
...
# Save the plots
save_data(niches_statistics, here(images_dir, stat_niches_dir))
