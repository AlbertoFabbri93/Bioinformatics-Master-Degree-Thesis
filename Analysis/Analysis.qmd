...
for (cluster in names(GSEA_results)) {
  ...
  if (nrow(gsea_simplified@result) > 0) {
    ...
    GSEA_plots[[cluster]] <- GSEADotPlot
    GSEA_tibbles[[cluster]] <- as_tibble(gsea_simplified@result)
  } else {
...
niches <- unique(breast_cancer_patients_all_cells$niches)
# Print the niches
print(sort(niches))
...
# Save the plots
save_plots(niches_barplot_list, here(images_dir, "Integration", "Niches_barplots"))
```
