...
### COMPARE UNANNOTATED CLUSTERING ###
# generate_compare_clustering_methods_plot(patient_1_data, louvain_cluster_var, louvain_cluster_name,  ist_cluster_var, ist_cluster_name)
...
# Extract patient data
patient_2_data <- extract_patient_data(breast_cancer_patients, this_patient_num)
# Create empty vector to store the plots
patient_2_plots <- c()

# Analyze protein data
patient_2_data <- analyze_proteins(patient_2_data)

### FILTERING - PROTEINS CLUSTERING ###
# Elbow plot
patient_2_plots <- c(patient_2_plots, generate_variance_elbow_plots(patient_2_data, "pca_proteins", 9))

# Feature plot of proteins, dyes and area
proteins_features_plots <- generate_feature_plot(
  patient_data = patient_2_data,
  reduction = "umap_proteins",
  features = c("Mean.PanCK", "Mean.CD45", "Mean.CD68", "Mean.Membrane", "Mean.DAPI", "Area" ),
  max_cutoff = "q95")
patient_2_plots <- c(patient_2_plots, proteins_features_plots)

# Generate the color palette for the protein clusters
this_patient_protein_color_table <- generate_colors_lookup_table(patient_2_data, protein_cluster_var, known_clusters_colors)
# Spatial plot with clusters colors
patient_2_plots <- c(patient_2_plots, generate_spatial_plots(
  patient_2_data,
  protein_cluster_var,
  cluster_name = protein_cluster_name,
  color_lookup_table = this_patient_protein_color_table))
# Umap plot with clusters colors
patient_2_plots <- c(patient_2_plots, generate_umap(
  patient_2_data,
  protein_cluster_var, 
  "umap_proteins",
  cluster_name = protein_cluster_name,
...
# Remove red blood cells
patient_2_data <- remove_clusters(patient_2_data, "protein_clusters", c(6))

### UNANNOTATED LOUVAIN CLUSTERING ###
# Analyze RNA data
patient_2_data <- normalize_cluster_data(patient_2_data)
patient_2_plots <- c(patient_2_plots, generate_variance_elbow_plots(patient_2_data, "pca_rna", 50))
# Generate the colors for the louvain unannotated clusters
this_patient_louvain_color_table <- generate_colors_lookup_table(patient_2_data, louvain_cluster_var, known_clusters_colors)
# Spatial plot with louvain clusters colors
patient_2_plots <- c(patient_2_plots, generate_spatial_plots(
  patient_2_data,
  louvain_cluster_var,
...
patient_2_plots <- c(patient_2_plots, generate_umap(
  ...
  color_lookup_table = this_patient_louvain_color_table))
# Heatmap of the louvain clusters
patient_2_plots <- c(patient_2_plots, generate_dyn_text_heatmap(
  patient_2_data,
  louvain_cluster_var,
  "RNA",
  cluster_name = louvain_cluster_name,
...
### UNANNOTATED INSITUTYPE CLUSTERING ###
# Execute insitutype
patient_2_ist_semisup <- run_ist_semisup_extract_data(patient_2_data)
patient_2_data[[ist_cluster_var]] <- patient_2_ist_semisup$clust
# Colors for the ist plots
this_patient_ist_color_table <- generate_colors_lookup_table(patient_2_data, ist_cluster_var, known_clusters_colors)
# Spatial plot with louvain clusters colors
patient_2_plots <- c(patient_2_plots, generate_spatial_plots(
  patient_2_data,
  ist_cluster_var,
...
patient_2_plots <- c(patient_2_plots, generate_umap(
  ...
  color_lookup_table = this_patient_ist_color_table))
# Generate the flightpath plot
patient_2_plots <- c(patient_2_plots, generate_flightpath(
  patient_2_ist_semisup,
  patient_num = "2",
  color_lookup_table = this_patient_ist_color_table))
# Generate the heatmap of the non-annotated clusters
patient_2_plots <- c(patient_2_plots, generate_dyn_text_heatmap(
...
### ANNOTATED LOUVAIN CLUSTERING ###
# Luovain clusters annotations
patient_2_data$rna_louvain_clusters_ann <- case_match(
  patient_2_data$rna_louvain_clusters,
  c("0", "1", "2") ~ "Tumor",
  "3" ~ "Fibroblast",
  "4" ~ "Macrophage",
  "5" ~ "Endothelial",
...
### ANNOTATED INSITUTYPE CLUSTERING ###
# Annotate the insitutype clusters
patient_2_data$ist_semisup_clusters_ann <- case_match(
  patient_2_data$ist_semisup_clusters,
  c("a", "b") ~ "Tumor",
  "c" ~ "Dendritic cell",
  c("d", "e") ~ "Fibroblast",
  .default = unname(patient_2_data$ist_semisup_clusters)
)
# Spatial plot with annotated insitutype clusters colors
patient_2_plots <- c(patient_2_plots, generate_spatial_plots(
  patient_data = patient_2_data,
